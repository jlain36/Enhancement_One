/* CS-330_Final_Project.cpp
   Author: John Lain
   Original Creation Date: 4/16/2021
   Enhancement Date: 11/10/2021

   Enhancement One for CS-499 Computer Science Capstone:

   1. Created the chalk board primnitive object.
   2. Created the chalk board leg(s) primitive objects.
   3. Created the chalk board leg support(s) primitive objects.
   4. Added primitive shape texturing.
   5. Added additional comments to the code.
   6. Added user authentication.
...
// Header file modified to incorporate UP/DOWN keyboard enums
...
namespace
{
    const char* const WINDOW_TITLE = "CS-499 Enhancement One"; // Macro for window title
    ...
    glm::vec3 gLightColor2(0.45f, 0.45f, 0.45f);
...
void Base();
void Chalk_Board();
void Chalk_Board_Leg_1();
void Chalk_Board_Leg_2();
void Chalk_Board_Leg_Support_1();
void Chalk_Board_Leg_Support_2();
...
int main(int argc, char* argv[])
{
    // User authentication
    //--------------------
    // Initialize variables and set default username and password
    string userName = "";
    ...
    // While the username is incorrect keep prompting for the username
    while (userName != UserName) {
        cout << "The username is incorrect. Re-enter username: ";
        cin >> userName;
    }
    ...
    // While the password is incorrect keep prompting for the password
    while (passWord != PassWord) {
        cout << "The password is incorrect. Re-enter password: ";
        cin >> passWord;
    }
    ...
    // Chalk Board texture
    texFilename = "Images/chalk_board.png";
    if (!UCreateTexture(texFilename, gTextureIdChalkBoard))
    {
        cout << "Failed to load texture " << texFilename << endl;
        return EXIT_FAILURE;
    }
    ...
    glUniform1i(glGetUniformLocation(gProgramId, "uChalkBoard"), 6);
    ...
    // render loop
    // -----------
    // Render until the window is closed or the escape key is pressed
    while (!glfwWindowShouldClose(gWindow))
    {
        ...
        Chalk_Board_Leg_1();
        Chalk_Board_Leg_2();
        Chalk_Board_Leg_Support_1();
        Chalk_Board_Leg_Support_2();
    ...
    UDestroyTexture(gTextureIdChalkBoard);
...
bool UInitialize(int argc, char* argv[], GLFWwindow** window)
{
    ...
    * window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, WINDOW_TITLE, NULL, NULL);
    if (*window == NULL)
    {
        // Report error and terminate window
        std::cout << "Failed to create GLFW window" << std::endl;
        glfwTerminate();
        return false;
    }

    // Make the current window context and resize window
    glfwMakeContextCurrent(*window);
    glfwSetFramebufferSizeCallback(*window, UResizeWindow);

    //Mouse position and scroll callbacks
...
void UProcessInput(GLFWwindow* window)
{
    ...
    // Boolean to toggle orthographic view
    ...
    // Changes the viewport perspective (while "P" key is pressed: Orthgraphic view)
...
void UMousePositionCallback(GLFWwindow* window, double xpos, double ypos)
{
    ...
    // Increment camera's position WRT mouse movement
    xoffset += gCamera.MouseSensitivity;
    yoffset += gCamera.MouseSensitivity;

    // Assign the X.Y position
    gLastX = xpos;
    gLastY = ypos;

    // Update the scene position WRT the mouse position
...
// Chalk Board function
void Chalk_Board()
{
    // Position variables
    const float xsize = 2.5f;
    const float ysize = 1.6f;
    const float zsize = -4.5f;
    ...
    // Bind texture unit 6
    glBindTexture(GL_TEXTURE_2D, gTextureIdChalkBoard);

    // 1. Scales the object 
    glm::mat4 scale = glm::scale(glm::vec3(0.1f, 2.5f, 4.0f));
...
// Chalk Board Leg 1 function
void Chalk_Board_Leg_1()
{
    // Position variables
    const float xsize = 0.79f;
    const float ysize = 0.849f;
    const float zsize = -5.54f;
    ...
    // 1. Scales the object 
    glm::mat4 scale = glm::scale(glm::vec3(0.1f, 4.01f, 0.1f));
...
// Chalk Board Leg 2 function
void Chalk_Board_Leg_2()
{
    // Position variables
    const float xsize = 4.22f;
    const float ysize = 0.844f;
    const float zsize = -3.43f;
...
// Chalk Board Leg support 1 function
void Chalk_Board_Leg_Support_1()
{
    // Position variables
    const float xsize = 0.777f;
    const float ysize = -1.0f;
    const float zsize = -5.56f;
    ...
    // 1. Scales the object by 2
    glm::mat4 scale = glm::scale(glm::vec3(0.85f, 0.3f, 0.17f));
...
// Chalk Board Leg support 2 function
void Chalk_Board_Leg_Support_2()
{
    // Position variables
    const float xsize = 4.24f;
    const float ysize = -1.0f;
    const float zsize = -3.47f;
...
// Function to create the desktop/rectangle mesh
